...
ROOT_SCHEMA_REF = 'root-schema'
root_schema = core_schema.definition_reference_schema(schema_ref=ROOT_SCHEMA_REF)
SER_SCHEMA_REF = 'ser-schema'
ser_schema = core_schema.definition_reference_schema(schema_ref=SER_SCHEMA_REF)
...
def type_dict_schema(typed_dict) -> dict[str, Any]:  # noqa: C901
    ...
    for field_name, field_type in typed_dict.__annotations__.items():
        ...
        if type(field_type) == ForwardRef:
            ...
            for name, ref_schema in (('CoreSchema', root_schema), ('SerSchema', ser_schema)):
                if name == fr_arg or re.search(f'[^a-zA-Z]{name}', fr_arg):
                    if fr_arg == name:
                        schema = ref_schema
                    elif fr_arg == f'List[{name}]':
                        schema = {'type': 'list', 'items_schema': ref_schema}
                    elif fr_arg == f'Dict[str, {name}]':
                        schema = {'type': 'dict', 'keys_schema': {'type': 'str'}, 'values_schema': ref_schema}
                    elif fr_arg == f'Dict[Union[str, int], Union[str, int, {name}]]':
                        schema = {
                            'type': 'dict',
                            'keys_schema': {'type': 'union', 'choices': [{'type': 'str'}, {'type': 'int'}]},
                            'values_schema': {
                                'type': 'union',
                                'choices': [{'type': 'str'}, {'type': 'int'}, ref_schema],
                            },
                        }
                    else:
...
def main() -> None:
    ...
    schema_union = core_schema.SerSchema
    assert get_origin(schema_union) is Union, 'expected core_schema.SerSchema to be a Union'
    ser_schema_choices: list[core_schema.CoreSchema] = []
    for s in list(get_args(schema_union)):
        ser_schema_choices.append(get_schema(s))
    definitions.append(core_schema.union_schema(ser_schema_choices, ref=SER_SCHEMA_REF))

    schema_union = core_schema.CoreSchema
    assert get_origin(schema_union) is Union, 'expected core_schema.CoreSchema to be a Union'
    core_schema_choices: dict[str, core_schema.CoreSchema] = {}
    for s in list(get_args(schema_union)):
        type_annotation = s.__annotations__['type']
        m = re.search(r"Literal\['(.+?)']", type_annotation.__forward_arg__)
        assert m, f'Unknown schema type: {type_annotation}'
        key: str = m.group(1)
        core_schema_choices[key] = get_schema(s)
    definitions.append(core_schema.tagged_union_schema(core_schema_choices, 'type', ref=ROOT_SCHEMA_REF))

    schema = core_schema.definitions_schema(root_schema, definitions)
