...
impl BuildSerializer for LiteralSerializer {
    ...
    fn build(
        ...
        let expected: Py2<'_, PyList> = schema.get_as_req(intern2!(schema.py(), "expected"))?;
...
enum OutputValue<'py> {
    OkInt(i64),
    OkStr(Py2<'py, PyString>),
    Ok,
    Fallback,
}

impl LiteralSerializer {
    fn check<'py>(&self, value: &Py2<'py, PyAny>, extra: &Extra) -> PyResult<OutputValue<'py>> {
