                ...
                match extra.mode {
                    SerMode::Json => {
                        ...
                        for (index, iter_result) in py_iter.enumerate() {
                            ...
                            let op_next = self.filter.index_filter(py, index, include, exclude, None)?;
        ...
        match value.downcast::<PyIterator>() {
            Ok(py_iter) => {
                ...
                for (index, iter_result) in py_iter.enumerate() {
                    ...
                    let op_next = self
                        .filter
                        .index_filter(value.py(), index, include, exclude, None)
...
impl SerializationIterator {
    ...
    fn __next__(&mut self, py: Python) -> PyResult<Option<PyObject>> {
        ...
        for iter_result in iterator {
            let element = iter_result?;
            let filter = self.filter.index_filter(py, self.index, include, exclude, None)?;
            self.index += 1;
