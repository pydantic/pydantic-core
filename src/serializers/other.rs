...
impl BuildSerializer for ChainBuilder {
    ...
    fn build(schema: &PyDict, config: Option<&PyDict>) -> PyResult<CombinedSerializer> {
        let last_schema = schema
            ...
            .unwrap()
            .cast_as()?;
        CombinedSerializer::build(last_schema, config)
    }
}
...
impl BuildSerializer for FunctionBuilder {
    ...
    fn build(schema: &PyDict, config: Option<&PyDict>) -> PyResult<CombinedSerializer> {
        ...
        if mode == "before" || mode == "after" {
            ...
            CombinedSerializer::build(schema, config)
        } else {
            AnySerializer::build(schema, config)
        }
    }
}
...
impl BuildSerializer for CustomErrorBuilder {
    ...
    fn build(schema: &PyDict, config: Option<&PyDict>) -> PyResult<CombinedSerializer> {
        ...
        CombinedSerializer::build(sub_schema, config)
    }
}
