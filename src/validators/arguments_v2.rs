...
#[derive(Debug)]
pub struct ArgumentsV2Validator {
...
impl BuildValidator for ArgumentsV2Validator {
...
impl_py_gc_traverse!(ArgumentsV2Validator { parameters });

impl Validator for ArgumentsV2Validator {
    fn validate<'py>(
        &self,
        py: Python<'py>,
        ...
        if let Ok(dict) = args_dict {
            ...
            for parameter in self.parameters.iter() {
                ...
                if let Some((lookup_path, dict_value)) = dict.get_item(&parameter.lookup_key)? {
                    match parameter.mode {
                        ...
                        // ParameterMode::VarArgs => match dict_value.validate_iter() {
                        //     Ok(iterable) => iterable.next(),
                        //     Err(err) => return Err(err),
                        // },
                        ParameterMode::VarArgs => todo!(),
                        ParameterMode::KeywordOnly => {
                            match parameter.validator.validate(py, dict_value.borrow_input(), state) {
                                Ok(value) => output_kwargs
                                    .set_item(PyString::new(py, parameter.name.as_str()).unbind(), value)?,
                                Err(ValError::LineErrors(line_errors)) => {
                                    errors.extend(line_errors.into_iter().map(|err| {
                                        lookup_path.apply_error_loc(err, self.loc_by_alias, &parameter.name)
                                    }));
            ...
            // Validating from `ArgsKwargs`. This is only possible with Python inputs.

            let args = input.validate_args_v2()?;
            let mut output_args: Vec<PyObject> = Vec::with_capacity(self.positional_params_count);
            let output_kwargs = PyDict::new(py);
            let mut errors: Vec<ValLineError> = Vec::new();
                                ...
                                Some(var_kwargs_parameter) => {
                                    match var_kwargs_parameter.mode {
                                        ParameterMode::VarKwargsUniform => {
                                            match var_kwargs_parameter.validator.validate(
                                                py,
                                                value.borrow_input(),
                                                state,
                                            ) {
                                                Ok(value) => {
                                                    output_kwargs.set_item(
