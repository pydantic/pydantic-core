...
impl BuildValidator for UnionValidator {
    ...
    fn build(
        ...
        let choices: Vec<(CombinedValidator, Option<String>)> = schema
            .get_as_req::<Py2<'_, PyList>>(intern2!(py, "choices"))?
            .iter()
            .map(|choice| {
        ...
        let auto_collapse = || schema.get_as_req(intern2!(py, "auto_collapse")).unwrap_or(true);
        let mode = schema
...
impl Discriminator {
    fn new(py: Python, raw: &Py2<'_, PyAny>) -> PyResult<Self> {
        if raw.is_callable() {
...
impl BuildValidator for TaggedUnionValidator {
    ...
    fn build(
        ...
        let py = schema.py();
        let discriminator = Discriminator::new(py, &schema.get_as_req(intern2!(py, "discriminator"))?)?;
        let discriminator_repr = discriminator.to_string_py(py)?;

        let choices = PyDict::new2(py);
        ...
        let schema_choices: Py2<PyDict> = schema.get_as_req(intern2!(py, "choices"))?;
        ...
        let key = intern2!(py, "from_attributes");
    ...
    fn validate<'data>(
        ...
        match self.discriminator {
            Discriminator::LookupKey(ref lookup_key) => {
                ...
                self.find_call_validator(py, tag.attach(py), input, state)
            }
...
impl TaggedUnionValidator {
    fn self_schema_tag<'data>(
        ...
        py: Python<'data>,
        input: &'data impl Input<'data>,
    ) -> ValResult<Py2<'data, PyString>> {
        let dict = input.strict_dict()?;
        let tag = match &dict {
            GenericMapping::PyDict(dict) => match dict.get_item(intern2!(py, "type"))? {
        ...
        if tag == "function" || tag == "tuple" {
            let mode = match &dict {
                GenericMapping::PyDict(dict) => match dict.get_item(intern2!(py, "mode"))? {
                    Some(m) => Some(m.downcast_into::<PyString>()?),
                    None => None,
                },
                _ => unreachable!(),
            };
            if tag == "function" {
                ...
                match mode.to_str()? {
                    "plain" => Ok(intern2!(py, "function-plain").clone()),
                    "wrap" => Ok(intern2!(py, "function-wrap").clone()),
                    _ => Ok(intern2!(py, "function").clone()),
                }
            } else {
                // tag == "tuple"
                if let Some(mode) = mode {
                    if mode.to_str()? == "positional" {
                        return Ok(intern2!(py, "tuple-positional").clone());
                    }
                }
                Ok(intern2!(py, "tuple-variable").clone())
            }
        } else {
            Ok(PyString::new2(py, tag))
        }
    }
