...
impl BuildValidator for CallValidator {
    ...
    fn build(
        ...
        let arguments_validator = Box::new(build_validator(arguments_schema, user_config, definitions)?);

        let return_schema = schema.get_item(intern!(py, "return_schema"));
        let return_validator = match return_schema {
            Some(return_schema) => Some(Box::new(build_validator(return_schema, user_config, definitions)?)),
            None => None,
        };
