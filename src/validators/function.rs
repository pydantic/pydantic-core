...
fn destructure_function_schema(schema: &Py2<'_, PyDict>) -> PyResult<FunctionInfo> {
    let func_dict: Py2<'_, PyDict> = schema.get_as_req(intern2!(schema.py(), "function"))?;
    let function = func_dict.get_as_req(intern2!(schema.py(), "function"))?;
    let func_type: Py2<'_, PyString> = func_dict.get_as_req(intern2!(schema.py(), "type"))?;
    ...
    let field_name = func_dict
        .get_as::<&PyString>(intern2!(schema.py(), "field_name"))?
        .map(Into::into);
    Ok(FunctionInfo {
        function,
        field_name,
    ...
    ($impl_name:ident, $name:literal) => {
        impl BuildValidator for $impl_name {
            ...
            fn build(
                ...
                let py = schema.py();
                let validator = build_validator(&schema.get_as_req(intern2!(py, "schema"))?, config, definitions)?;
                let func_info = destructure_function_schema(schema)?;
                let name = format!(
                    "{}[{}(), {}]",
                    $name,
                    function_name(func_info.function.attach(py))?,
                    validator.get_name()
                );
                Ok(Self {
...
impl BuildValidator for FunctionPlainValidator {
    ...
    fn build(
        ...
        Ok(Self {
            ...
            name: format!(
                "function-plain[{}()]",
                function_name(function_info.function.attach(py))?
            ),
...
impl BuildValidator for FunctionWrapValidator {
    ...
    fn build(
        ...
        let validator = build_validator(&schema.get_as_req(intern2!(py, "schema"))?, config, definitions)?;
        let function_info = destructure_function_schema(schema)?;
        let hide_input_in_errors: bool = config.get_as(intern2!(py, "hide_input_in_errors"))?.unwrap_or(false);
        let validation_error_cause: bool = config.get_as(intern2!(py, "validation_error_cause"))?.unwrap_or(false);
        Ok(Self {
            ...
            name: format!("function-wrap[{}()]", function_name(function_info.function.attach(py))?),
            field_name: function_info.field_name.clone(),
            info_arg: function_info.info_arg,
...
impl Validator for FunctionWrapValidator {
    fn validate<'data>(
        ...
        let handler = Py2::new(py, handler)?;
    ...
    fn validate_assignment<'data>(
        ...
        self._validate(Py2::new(py, handler)?.as_any(), py, obj, state)
    }
...
impl ValidatorCallable {
    fn __call__(
        ...
        outer_location: Option<Py2<'_, PyAny>>,
    ) -> PyResult<PyObject> {
        let outer_location = outer_location.as_ref().map(AsLocItem::as_loc_item);
