...
impl DefaultType {
    pub fn new(schema: &Py2<'_, PyDict>) -> PyResult<Self> {
        let py = schema.py();
        match (
            schema.get_as(intern2!(py, "default"))?,
            schema.get_as(intern2!(py, "default_factory"))?,
        ) {
...
impl BuildValidator for WithDefaultValidator {
    ...
    fn build(
        ...
        let on_error = match schema
            .get_as::<Py2<'_, PyString>>(intern2!(py, "on_error"))?
            .as_ref()
            .map(|s| s.to_str())
            .transpose()?
        {
        ...
        Ok(Self {
            ...
            on_error,
            validator,
            validate_default: schema_or_config_same(schema, config, intern2!(py, "validate_default"))?.unwrap_or(false),
            copy_default,
            name,
        }
        .into())
        ...
        match self.default.default_value(py)? {
            Some(stored_dft) => {
                ...
                if self.validate_default {
                    match self.validate(py, dft.attach(py), state) {
                        Ok(v) => Ok(Some(v)),
                        Err(e) => {
