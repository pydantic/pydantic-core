...
pub enum GenericIterable<'a> {
    ...
    Tuple(Py2<'a, PyTuple>),
    Set(Py2<'a, PySet>),
    FrozenSet(Py2<'a, PyFrozenSet>),
    Dict(Py2<'a, PyDict>),
    // Treat dict values / keys / items as generic iterators
    // since PyPy doesn't export the concrete types
    DictKeys(Py2<'a, PyIterator>),
    DictValues(Py2<'a, PyIterator>),
    DictItems(Py2<'a, PyIterator>),
    Mapping(Py2<'a, PyMapping>),
    PyString(Py2<'a, PyString>),
    Bytes(Py2<'a, PyBytes>),
    PyByteArray(Py2<'a, PyByteArray>),
    Sequence(Py2<'a, PySequence>),
    Iterator(Py2<'a, PyIterator>),
...
impl<'a, 'py: 'a> GenericIterable<'a> {
    pub fn as_sequence_iterator(
        ...
        match self {
            ...
            GenericIterable::JsonString(s) => Ok(Box::new(PyString::new2(py, s).iter()?)),
        }
    }
}
...
pub enum GenericMapping<'a> {
    PyDict(Py2<'a, PyDict>),
    PyMapping(Py2<'a, PyMapping>),
    StringMapping(Py2<'a, PyDict>),
    PyGetAttr(Py2<'a, PyAny>, Option<Py2<'a, PyDict>>),
    JsonObject(&'a JsonObject),
}

macro_rules! derive_from {
    ($enum:ident, $key:ident, $type:ty $(, $extra_types:ident )*) => {
        impl<'a> From<Py2<'a, $type>> for $enum<'a> {
            fn from(s: Py2<'a, $type>) -> $enum<'a> {
...
impl<'py> Iterator for DictGenericIterator<'py> {
    type Item = ValResult<(Py2<'py, PyAny>, Py2<'py, PyAny>)>;
...
pub struct AttributesGenericIterator<'py> {
    object: Py2<'py, PyAny>,
    attributes_iterator: PyListIter<'py>,
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if !name_cow.as_ref().starts_with('_') {
            ...
            if let Ok(attr) = self.object.getattr(name_cow.as_ref()) {
                ...
                let is_bound = matches!(attr.hasattr(intern2!(attr.py(), "__self__")), Ok(true));
...
impl From<&Py2<'_, PyAny>> for GenericIterator {
    fn from(obj: &Py2<'_, PyAny>) -> Self {
...
impl GenericPyIterator {
    pub fn next<'py>(&mut self, py: Python<'py>) -> PyResult<Option<(Py2<'py, PyAny>, usize)>> {
        match self.iter.attach(py).next() {
            Some(Ok(next)) => {
...
impl<'a> EitherString<'a> {
    ...
    pub fn as_py_string(&'a self, py: Python<'a>) -> Py2<'a, PyString> {
        match self {
            Self::Cow(cow) => PyString::new2(py, cow),
...
impl<'a> From<Py2<'a, PyString>> for EitherString<'a> {
    fn from(date: Py2<'a, PyString>) -> Self {
        Self::Py(date)
    }
}
...
pub fn py_string_str<'a>(py_str: &'a Py2<'_, PyString>) -> ValResult<&'a str> {
...
impl<'a> From<&'_ Py2<'a, PyBytes>> for EitherBytes<'a> {
    fn from(bytes: &'_ Py2<'a, PyBytes>) -> Self {
        Self::Py(bytes.clone())
    }
}
...
impl<'a> IntoPy<PyObject> for EitherBytes<'a> {
    fn into_py(self, py: Python<'_>) -> PyObject {
        match self {
            EitherBytes::Cow(bytes) => PyBytes::new2(py, &bytes).into_py(py),
...
impl<'a> EitherInt<'a> {
    pub fn upcast(py_any: &Py2<'a, PyAny>) -> ValResult<Self> {
        ...
        match self {
            ...
            EitherInt::BigInt(u) => match i64::try_from(u) {
                ...
                Err(e) => Err(ValError::new(
                    ErrorTypeDefaults::IntParsingSize,
                    e.into_original().into_py(py).attach(py),
                )),
            },
            EitherInt::Py(i) => i
                .extract()
