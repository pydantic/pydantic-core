...
impl LookupKey {
    ...
    fn simple(py: Python, key: &str, opt_py_key: Option<Py2<'_, PyString>>) -> Self {
        let py_key = match &opt_py_key {
            Some(py_key) => py_key.clone(),
            None => PyString::new2(py, key),
        };
        Self::Simple {
            key: key.to_string(),
            py_key: py_key.into(),
...
impl LookupPath {
    fn from_str(py: Python, key: &str, py_key: Option<Py2<'_, PyString>>) -> Self {
    ...
    fn from_list(obj: &Py2<'_, PyAny>) -> PyResult<LookupPath> {
        let v = obj
            .downcast::<PyList>()?
            .iter()
            .enumerate()
...
impl PathItem {
    pub fn from_py(index: usize, obj: &Py2<'_, PyAny>) -> PyResult<Self> {
