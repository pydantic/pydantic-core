...
use super::error::PydanticSerdeError;
...
where
    R: std::io::Read,
{
    iter: LineColIterator<std::io::Bytes<R>>,
...
where
    R: io::Read,
{
    fn parse_str_bytes<'s, T, F>(&'s mut self, scratch: &'s mut Vec<u8>, validate: bool, result: F) -> Result<T>
    where
        T: 's,
    ...
    fn next(&mut self) -> Result<Option<u8>> {
        match self.ch.take() {
            ...
            None => match self.iter.next() {
                Some(Err(err)) => Err(PydanticSerdeError::io(err)),
    ...
    R: ?Sized + Read<'de>,
{
    ...
    Err(PydanticSerdeError::syntax(reason, position.line, position.column))
}
