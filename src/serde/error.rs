...
pub type Result<T> = result::Result<T, PydanticSerdeError>;

impl PydanticSerdeError {
...
impl From<PydanticSerdeError> for io::Error {
    fn from(j: PydanticSerdeError) -> Self {
...
impl PydanticSerdeError {
    ...
    where
        F: FnOnce(ErrorCode) -> PydanticSerdeError,
    {
        if self.err.line == 0 {
            f(self.err.code)
        } else {
...
impl serde::de::StdError for PydanticSerdeError {
...
impl Display for PydanticSerdeError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl Debug for PydanticSerdeError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "PydanticSerdeError({:?}, line: {}, column: {})",
...
impl de::Error for PydanticSerdeError {
    #[cold]
    fn custom<T: Display>(msg: T) -> PydanticSerdeError {
    ...
    fn invalid_type(unexp: de::Unexpected, exp: &dyn de::Expected) -> Self {
        if let de::Unexpected::Unit = unexp {
            PydanticSerdeError::custom(format_args!("invalid type: null, expected {exp}"))
        } else {
            PydanticSerdeError::custom(PydanticSerdeError::custom(format_args!(
...
impl ser::Error for PydanticSerdeError {
    #[cold]
    fn custom<T: Display>(msg: T) -> PydanticSerdeError {
        make_error(msg.to_string())
    }
...
// around erased-serde round-tripping the error through de::PydanticSerdeError::custom.
fn make_error(mut msg: String) -> PydanticSerdeError {
