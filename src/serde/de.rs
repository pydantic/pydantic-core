...
use super::error::{ErrorCode, PydanticSerdeError, Result};
use super::read::{self, Fused, Reference};
pub use super::read::{Read, SliceRead, StrRead};
use super::shared::{tri, TOKEN};
use serde::de::{self, Expected, IntoDeserializer, Unexpected};
...
impl<'de> serde::de::MapAccess<'de> for NumberDeserializer {
...
impl<'de> serde::Deserializer<'de> for NumberFieldDeserializer {
    type Error = PydanticSerdeError;
...
pub struct PythonDeserializer<R> {
    read: R,
    scratch: Vec<u8>,
    remaining_depth: u8,
}

impl<'de, R> PythonDeserializer<R>
where
    R: read::Read<'de>,
{
    ...
    ///
    ///   - PythonDeserializer::from_str
    ///   - PythonDeserializer::from_slice
    ///   - PythonDeserializer::from_reader
...
impl<R> PythonDeserializer<read::IoRead<R>>
where
    R: io::Read,
{
    ...
    pub fn from_reader(reader: R) -> Self {
        PythonDeserializer::new(read::IoRead::new(reader))
    }
}

impl<'a> PythonDeserializer<read::SliceRead<'a>> {
    /// Creates a JSON deserializer from a `&[u8]`.
    pub fn from_slice(bytes: &'a [u8]) -> Self {
        PythonDeserializer::new(read::SliceRead::new(bytes))
    }
}

impl<'a> PythonDeserializer<read::StrRead<'a>> {
    /// Creates a JSON deserializer from a `&str`.
    pub fn from_str(s: &'a str) -> Self {
        PythonDeserializer::new(read::StrRead::new(s))
    }
}
...
impl ParserNumber {
    ...
    fn invalid_type(self, exp: &dyn Expected) -> PydanticSerdeError {
        match self {
...
impl<'de, R: Read<'de>> PythonDeserializer<R> {
    /// The `PythonDeserializer::end` method should be called after a value has been fully deserialized.
    ...
    #[cold]
    fn error(&self, reason: ErrorCode) -> PydanticSerdeError {
        let position = self.read.position();
        PydanticSerdeError::syntax(reason, position.line, position.column)
    }

    /// Error caused by a byte from peek().
    #[cold]
    fn peek_error(&self, reason: ErrorCode) -> PydanticSerdeError {
    ...
    #[cold]
    fn peek_invalid_type(&mut self, exp: &dyn Expected) -> PydanticSerdeError {
        let err = match self.peek_or_null().unwrap_or(b'\x00') {
            b'I' => {
                self.eat_char();
                if let Err(err) = self.parse_ident(b"nfinity") {
                    return err;
                }
                de::Error::invalid_type(Unexpected::Float(::std::f64::NEG_INFINITY), exp)
            }
            b'N' => {
                self.eat_char();
            ...
            b'-' => {
                ...
                match self.peek_or_null().unwrap_or(b'\x00') {
                    ...
                    _ => match self.parse_any_number(false) {
        ...
        V: de::Visitor<'de>,
    {
        ...
        let value = match peek {
            b'-' => {
                ...
                match tri!(self.peek()) {
                    Some(b'I') => {
                        ...
                        tri!(Ok(ParserNumber::F64(::std::f64::NEG_INFINITY))).visit(visitor)
                    }
                    _ => tri!(self.parse_integer(false)).visit(visitor),
                }
            }
            ...
            b'I' => {
                self.eat_char();
                tri!(self.parse_ident(b"nfinity"));
                tri!(Ok(ParserNumber::F64(::std::f64::INFINITY))).visit(visitor)
            }
            b'N' => {
                self.eat_char();
    ...
    #[cold]
    fn fix_position(&self, err: PydanticSerdeError) -> PydanticSerdeError {
                    ...
                    match tri!(self.peek_or_null()) {
                        c @ b'0'..=b'9' => {
                            ...
                            if overflow!(significand * 10 + digit, u64::max_value()) {
                                return Ok(ParserNumber::F64(tri!(self.parse_long_integer(positive, significand),)));
                            }
    ...
    fn parse_decimal(&mut self, positive: bool, mut significand: u64, mut exponent: i32) -> Result<f64> {
        self.eat_char();
    ...
    fn f64_from_parts(&mut self, positive: bool, significand: u64, mut exponent: i32) -> Result<f64> {
    ...
    #[cold]
    #[inline(never)]
    fn parse_exponent_overflow(&mut self, positive: bool, zero_significand: bool, positive_exp: bool) -> Result<f64> {
...
impl<'de, 'a, R: Read<'de>> de::Deserializer<'de> for &'a mut PythonDeserializer<R> {
    type Error = PydanticSerdeError;
        ...
        V: de::Visitor<'de>,
    {
        ...
        let value = match peek {
            b'I' => {
                self.eat_char();
                tri!(self.parse_ident(b"nfinity"));
                visitor.visit_f64(::std::f64::INFINITY)
            }
            b'N' => {
                self.eat_char();
                tri!(self.parse_ident(b"aN"));
            ...
            b'-' => {
                ...
                match tri!(self.peek()) {
                    Some(b'I') => {
                        self.eat_char();
                        tri!(self.parse_ident(b"nfinity"));
                        visitor.visit_f64(::std::f64::NEG_INFINITY)
                    }
...
struct SeqAccess<'a, R: 'a> {
    de: &'a mut PythonDeserializer<R>,
    first: bool,
}

impl<'a, R: 'a> SeqAccess<'a, R> {
    fn new(de: &'a mut PythonDeserializer<R>) -> Self {
    ...
    T: de::Deserialize<'de>,
{
    ...
    /// Typically it is more convenient to use one of these methods instead:
    ///
    ///   - PythonDeserializer::from_str(...).into_iter()
    ///   - PythonDeserializer::from_slice(...).into_iter()
    ///   - PythonDeserializer::from_reader(...).into_iter()
    pub fn new(read: R) -> Self {
        let offset = read.byte_offset();
        StreamDeserializer {
            de: PythonDeserializer::new(read),
            offset,
            failed: false,
            output: PhantomData,
            lifetime: PhantomData,
...
where
    ...
    T: de::Deserialize<'de>,
{
    let mut de = PythonDeserializer::new(read);
