...
def test_build_error_internal():
    with pytest.raises(SchemaError, match='Value should be a valid integer, unable to parse string as an integer'):
...
def test_schema_wrong_type():
    ...
    assert exc_info.value.args[0] == (
        'Invalid Schema:\n  Value should be a valid dictionary [kind=dict_type, input_value=1, input_type=int]'
    )
