...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param((1,), Err('Value should be a valid time [kind=time_type'), id='tuple'),
        pytest.param(date(2022, 6, 8), Err('Value should be a valid time [kind=time_type'), id='date'),
        pytest.param(datetime(2022, 6, 8), Err('Value should be a valid time [kind=time_type'), id='datetime'),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            '12:13:14.1234561',
            Err(
                'Value should be in a valid time format, '
        ...
        pytest.param(
            86400,
            Err(
                'Value should be in a valid time format, numeric times may not exceed 86,399 seconds [kind=time_parsing'
            ),
            id='too-high',
        ),
        pytest.param(
            -1, Err('Value should be in a valid time format, time in seconds should be positive'), id='negative'
        ),
        ...
        pytest.param(True, Err('Value should be a valid time [kind=time_type'), id='bool'),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('12:13:14', Err('Value should be a valid time [kind=time_type')),
        (b'12:13:14', Err('Value should be a valid time [kind=time_type')),
        (1654646400, Err('Value should be a valid time [kind=time_type')),
        (True, Err('Value should be a valid time [kind=time_type')),
        (date(2022, 6, 8), Err('Value should be a valid time [kind=time_type')),
        (datetime(2022, 6, 8), Err('Value should be a valid time [kind=time_type')),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('"foobar"', Err('Value should be in a valid time format, invalid character in hour [kind=time_parsing,')),
        ('123', Err('Value should be a valid time [kind=time_type')),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'le': time(1)}, '01:01', Err('Value should be less than or equal to 01:00:00')),
        ({'le': time(1)}, time(1), time(1, 0)),
        ({'le': time(1)}, time(1, 1), Err('Value should be less than or equal to 01:00:00')),
        ({'lt': time(1)}, '00:59', time(0, 59)),
        ({'lt': time(1)}, '01:00', Err('Value should be less than 01:00:00')),
        ({'ge': time(1)}, '01:00', time(1)),
        ({'ge': time(1)}, '00:59', Err('Value should be greater than or equal to 01:00:00')),
        ({'gt': time(12, 13, 14, 123_456)}, '12:13:14.123457', time(12, 13, 14, 123_457)),
        ({'gt': time(12, 13, 14, 123_456)}, '12:13:14.123456', Err('Value should be greater than 12:13:14.123456')),
        ({'gt': '12:13:14.123456'}, '12:13:14.123456', Err('Value should be greater than 12:13:14.123456')),
    ],
)
    ...
    assert exc_info.value.errors() == [
        {
            'kind': 'greater_than',
            'loc': [],
            'message': 'Value should be greater than 12:13:14.123456',
            'input_value': '12:13',
...
def test_invalid_constraint():
    with pytest.raises(SchemaError, match='Value should be in a valid time format'):
