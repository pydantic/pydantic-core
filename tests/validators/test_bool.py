...
@pytest.mark.parametrize(
    ...
    [
        ...
        (2, Err('Value should be a valid boolean, unable to interpret input [kind=bool_parsing, input_value=2')),
        ([], Err('Value should be a valid boolean [kind=bool_type, input_value=[], input_type=list]')),
        (1.1, Err('Value should be a valid boolean [kind=bool_type, input_value=1.1, input_type=float]')),
...
def test_bool_strict(py_and_json: PyAndJson):
    ...
    error_message = "Value should be a valid boolean [kind=bool_type, input_value='true', input_type=str]"
    ...
    assert exc_info.value.errors() == [
        {
            'kind': 'bool_parsing',
            'loc': [],
            'message': 'Value should be a valid boolean, unable to interpret input',
            'input_value': 'wrong',
        }
    ]
...
def test_bool_key(py_and_json: PyAndJson):
    ...
    with pytest.raises(ValidationError, match='Value should be a valid boolean'):
