...
def test_allow_extra_invalid():
    with pytest.raises(SchemaError, match='extra_schema can only be used if extra_behavior=allow'):
        SchemaValidator(
            {'type': 'typed-dict', 'fields': {}, 'extra_schema': {'type': 'int'}, 'extra_behavior': 'ignore'}
        )
...
@pytest.mark.parametrize(
    'extra_schema_kw, expected_extra_value',
    [({}, '123'), ({'extra_schema': None}, '123'), ({'extra_schema': core_schema.int_schema()}, 123)],
    ids=['extra_schema=unset', 'extra_schema=None', 'extra_schema=int'],
)
def test_extra_behavior_allow(
    config: Union[core_schema.CoreConfig, None],
    schema_extra_behavior_kw: Dict[str, Any],
    extra_schema_kw: Dict[str, Any],
...
def test_leak_typed_dict():
    def fn():
        ...
        schema = core_schema.typed_dict_schema(
            {'f': core_schema.typed_dict_field(schema)}, extra_behavior='allow', extra_schema=schema
        )
