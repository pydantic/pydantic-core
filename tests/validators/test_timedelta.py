...
def test_timedelta(input_value, expected):
    v = SchemaValidator(schema=core_schema.timedelta_schema())
...
def test_timedelta_strict(input_value, expected):
    v = SchemaValidator(schema=core_schema.timedelta_schema(strict=True))
    if isinstance(expected, Err):
        with pytest.raises(ValidationError, match=re.escape(expected.message)):
            v.validate_python(input_value)
    else:
...
def test_timedelta_kwargs(kwargs: dict[str, Any], input_value, expected):
    v = SchemaValidator(schema=core_schema.timedelta_schema(**kwargs))
    if isinstance(expected, Err):
        with pytest.raises(ValidationError, match=re.escape(expected.message)):
            v.validate_python(input_value)
    else:
...
def test_timedelta_kwargs_strict():
    v = SchemaValidator(schema=core_schema.timedelta_schema(strict=True, le=timedelta(days=3)))
...
def test_dict_py():
    v = SchemaValidator(
        schema=core_schema.dict_schema(
            keys_schema=core_schema.timedelta_schema(), values_schema=core_schema.int_schema()
        )
    )
...
def test_union():
    v = SchemaValidator(
        schema=core_schema.union_schema(choices=[core_schema.str_schema(), core_schema.timedelta_schema()])
    )
    assert v.validate_python('P2DT1H') == 'P2DT1H'
    assert v.validate_python(timedelta(days=2, hours=1)) == timedelta(days=2, hours=1)

    v = SchemaValidator(
        schema=core_schema.union_schema(choices=[core_schema.timedelta_schema(), core_schema.str_schema()])
    )
    assert v.validate_python('P2DT1H') == 'P2DT1H'
    assert v.validate_python(timedelta(days=2, hours=1)) == timedelta(days=2, hours=1)
...
def test_pytimedelta_as_timedelta(constraint, expected_duration):
    v = SchemaValidator(schema=core_schema.timedelta_schema(gt=constraint))
...
def test_pandas():
    v = SchemaValidator(schema=core_schema.timedelta_schema(ge=timedelta(hours=2)))
