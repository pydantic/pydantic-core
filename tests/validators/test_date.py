...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param((1,), Err('Value should be a valid date [kind=date_type'), id='tuple'),
        ...
        pytest.param(253_402_300_800_000, Err('Value should be a valid date'), id='int-too-high'),
        # (-20_000_000_000, Err('format YYYY-MM-DD, dates before 1600 are not supported as unix timestamps')),
        pytest.param(-20_000_000_000, Err('Value should be a valid date'), id='int-too-low'),
        ...
        pytest.param(True, Err('Value should be a valid date'), id='bool'),
        pytest.param(time(1, 2, 3), Err('Value should be a valid date [kind=date_type'), id='time'),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('wrong', Err('Value should be a valid date or datetime, input is too short [kind=date_from_datetime_parsing')),
        ('2000-02-29', date(2000, 2, 29)),
        (
            '2001-02-29',
            Err(
                'Value should be a valid date or datetime, '
                'day value is outside expected range [kind=date_from_datetime_parsing'
            ),
        ),
        ([1], Err('Value should be a valid date [kind=date_type')),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('2022-06-08', Err('Value should be a valid date [kind=date_type')),
        (b'2022-06-08', Err('Value should be a valid date [kind=date_type')),
        (1654646400, Err('Value should be a valid date [kind=date_type')),
        (True, Err('Value should be a valid date [kind=date_type')),
        (datetime(2022, 6, 8), Err('Value should be a valid date [kind=date_type')),
    ],
    ids=repr,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            '"foobar"',
            Err('Value should be a valid date in the format YYYY-MM-DD, input is too short [kind=date_parsing,'),
        ),
        ('1654646400', Err('Value should be a valid date [kind=date_type')),
    ],
)
    ...
    assert exc_info.value.errors() == [
        {
            'kind': 'date_parsing',
            'loc': [],
            'message': 'Value should be a valid date in the format YYYY-MM-DD, input is too short',
            'input_value': 'foobar',
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {'le': date(2000, 1, 1)},
            '2000-01-02',
            Err('Value should be less than or equal to 2000-01-01 [kind=less_than_equal,'),
        ),
        ({'lt': '2000-01-01'}, '1999-12-31', date(1999, 12, 31)),
        ({'lt': '2000-01-01'}, '2000-01-01', Err('Value should be less than 2000-01-01 [kind=less_than,')),
        ({'ge': '2000-01-01'}, '2000-01-01', date(2000, 1, 1)),
        (
            {'ge': date(2000, 1, 1)},
            '1999-12-31',
            Err('Value should be greater than or equal to 2000-01-01 [kind=greater_than_equal,'),
        ),
        ({'gt': date(2000, 1, 1)}, '2000-01-02', date(2000, 1, 2)),
        ({'gt': date(2000, 1, 1)}, '2000-01-01', Err('Value should be greater than 2000-01-01 [kind=greater_than,')),
    ],
)
