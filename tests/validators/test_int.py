...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            12.5,
            Err('Value should be a valid integer, got a number with a fractional part [kind=int_from_float'),
            id='float-remainder',
        ),
        pytest.param(
            'wrong',
            Err('Value should be a valid integer, unable to parse string as an integer [kind=int_parsing'),
            id='string',
        ),
        pytest.param(None, Err('Value should be a valid integer [kind=int_type'), id='list'),
        pytest.param([1, 2], Err('Value should be a valid integer [kind=int_type'), id='list'),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            Decimal('1.001'),
            Err(
                'Value should be a valid integer, got a number with a fractional part '
        ...
        pytest.param(
            (1, 2),
            Err('Value should be a valid integer [kind=int_type, input_value=(1, 2), input_type=tuple]'),
            id='tuple',
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            42.0,
            Err('Value should be a valid integer [kind=int_type, input_value=42.0, input_type=float]'),
            id='float-exact',
        ),
        pytest.param(
            42.5,
            Err('Value should be a valid integer [kind=int_type, input_value=42.5, input_type=float]'),
            id='float-remainder',
        ),
        pytest.param(
            '42', Err("Value should be a valid integer [kind=int_type, input_value='42', input_type=str]"), id='string'
        ),
        pytest.param(
            True, Err('Value should be a valid integer [kind=int_type, input_value=True, input_type=bool]'), id='bool'
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'gt': 0}, 1, 1),
        ({'gt': 0}, 0, Err('Value should be greater than 0 [kind=greater_than, input_value=0, input_type=int]')),
        ({'le': 0}, 0, 0),
        ({'le': 0}, -1, -1),
        ({'le': 0}, 1, Err('Value should be less than or equal to 0')),
        ({'lt': 0}, 0, Err('Value should be less than 0')),
        ({'lt': 0}, 1, Err('Value should be less than 0')),
        ({'multiple_of': 5}, 15, 15),
        ({'multiple_of': 5}, 6, Err('Value should be a multiple of 5')),
    ],
    ids=repr,
)
...
def test_union_int(py_and_json: PyAndJson):
    ...
    assert exc_info.value.errors() == [
        {'kind': 'int_type', 'loc': ['int'], 'message': 'Value should be a valid integer', 'input_value': '5'},
        {
            'kind': 'multiple_of',
            'loc': ['constrained-int'],
            'message': 'Value should be a multiple of 7',
            'input_value': '5',
...
def test_long_int(py_and_json: PyAndJson):
    ...
    assert exc_info.value.errors() == [
        {
            'kind': 'int_nan',
            'loc': [],
            'message': 'Value should be a valid integer, got infinity',
            'input_value': '1' * 400,
    ...
    assert repr(exc_info.value) == (
        ...
        '  Value should be a valid integer, got infinity '
        '[kind=int_nan, '
...
def test_int_nan(py_and_json: PyAndJson):
    ...
    with pytest.raises(ValidationError, match='Value should be a valid integer, got negative infinity'):
        v.validate_test('-' + '1' * 400)

    with pytest.raises(ValidationError, match='Value should be a valid integer, got NaN'):
        v.validate_test('nan')
...
def test_int_key(py_and_json: PyAndJson):
    ...
    with pytest.raises(ValidationError, match='Value should be a valid integer'):
