...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, 1),
        (False, 0),
        ('wrong', Err('Value should be a valid number, unable to parse string as an number [kind=float_parsing')),
        ([1, 2], Err('Value should be a valid number [kind=float_type, input_value=[1, 2], input_type=list]')),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (42.0, 42.0),
        (42.5, 42.5),
        ('42', Err("Value should be a valid number [kind=float_type, input_value='42', input_type=str]")),
        (True, Err('Value should be a valid number [kind=float_type, input_value=True, input_type=bool]')),
    ],
    ids=repr,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'le': 0}, 0, 0),
        ({'le': 0}, -1, -1),
        ({'le': 0}, 0.1, Err('Value should be less than or equal to 0')),
        ({'lt': 0}, 0, Err('Value should be less than 0')),
        ({'lt': 0.123456}, 1, Err('Value should be less than 0.123456')),
        ({'multiple_of': 0.5}, 0.5, 0.5),
        ({'multiple_of': 0.5}, 1, 1),
        ({'multiple_of': 0.5}, 0.6, Err('Value should be a multiple of 0.5')),
    ],
)
...
def test_union_float(py_and_json: PyAndJson):
    ...
    assert exc_info.value.errors() == [
        {'kind': 'float_type', 'loc': ['float'], 'message': 'Value should be a valid number', 'input_value': '5'},
        {
            'kind': 'multiple_of',
...
def test_float_key(py_and_json: PyAndJson):
    ...
    with pytest.raises(ValidationError, match='Value should be a valid number'):
