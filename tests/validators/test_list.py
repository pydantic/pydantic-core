...
def test_list_strict():
    v = SchemaValidator(schema=cs.list_schema(items_schema=cs.int_schema(), strict=True))
...
def test_list_no_copy():
    v = SchemaValidator(schema=cs.list_schema())
...
def test_list_int(input_value, expected):
    v = SchemaValidator(schema=cs.list_schema(items_schema=cs.int_schema()))
    if isinstance(expected, Err):
...
def test_list_length_constraints(kwargs: dict[str, Any], input_value, expected):
    v = SchemaValidator(schema=cs.list_schema(**kwargs))
    if isinstance(expected, Err):
        with pytest.raises(ValidationError, match=re.escape(expected.message)):
            v.validate_python(input_value)
    else:
...
def test_list_length_constraints_omit(input_value, expected):
    v = SchemaValidator(
        schema=cs.list_schema(
            items_schema=cs.with_default_schema(schema=cs.int_schema(), on_error='omit'), max_length=4
        )
    )
    if isinstance(expected, Err):
        with pytest.raises(ValidationError, match=re.escape(expected.message)):
...
def test_length_ctx():
    v = SchemaValidator(schema=cs.list_schema(min_length=2, max_length=3))
    with pytest.raises(ValidationError) as exc_info:
        v.validate_python([1])
    # insert_assert(exc_info.value.errors(include_url=False))
    assert exc_info.value.errors(include_url=False) == [
...
def test_list_function():
    ...
    v = SchemaValidator(schema=cs.list_schema(items_schema=core_schema.with_info_plain_validator_function(f)))
...
def test_list_from_dict_items(input_value, items_schema, expected):
    v = SchemaValidator(schema=cs.list_schema(items_schema=items_schema))
...
def test_bad_iter(items_schema):
    ...
    v = SchemaValidator(schema=cs.list_schema(items_schema={'type': items_schema}))
...
def test_list_allowed_inputs_python(testcase: ListInputTestCase):
    v = SchemaValidator(schema=core_schema.list_schema(core_schema.int_schema(), strict=testcase.strict))
