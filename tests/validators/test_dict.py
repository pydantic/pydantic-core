...
def test_dict(py_and_json: PyAndJson):
    ...
    with pytest.raises(ValidationError, match='Value should be a valid dictionary'):
        v.validate_test([])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({}, {}),
        ('foobar', Err("Value should be a valid dictionary [kind=dict_type, input_value='foobar', input_type=str]")),
        ([], Err('Value should be a valid dictionary [kind=dict_type,')),
        ([('x', 'y')], Err('Value should be a valid dictionary [kind=dict_type,')),
        ([('x', 'y'), ('z', 'z')], Err('Value should be a valid dictionary [kind=dict_type,')),
        ((), Err('Value should be a valid dictionary [kind=dict_type,')),
        ((('x', 'y'),), Err('Value should be a valid dictionary [kind=dict_type,')),
        ((type('Foobar', (), {'x': 1})()), Err('Value should be a valid dictionary [kind=dict_type,')),
    ],
    ids=repr,
)
...
def test_dict_value_error(py_and_json: PyAndJson):
    ...
    with pytest.raises(ValidationError, match='Value should be a valid integer') as exc_info:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'min_items': 3}, {1: '2', 3: '4'}, Err('Input should have at least 3 items [kind=too_short')),
        ({'max_items': 4}, {'1': 1, '2': 2, '3': 3.0}, {'1': 1, '2': 2, '3': 3.0}),
        (
            {'max_items': 3},
            {'1': 1, '2': 2, '3': 3.0, '4': [1, 2, 3, 4]},
            Err('Input should have at most 3 items [kind=too_long'),
        ),
    ],
)
