...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "foobar",
            Err(
                "Input should be a valid dictionary [type=dict_type, input_value='foobar', input_type=str]"
            ),
        ),
...
def test_dict_cases(input_value, expected):
    v = SchemaValidator(
        cs.dict_schema(keys_schema=cs.str_schema(), values_schema=cs.str_schema())
    )
...
def test_mapping():
    ...
    v = SchemaValidator(
        cs.dict_schema(keys_schema=cs.int_schema(), values_schema=cs.int_schema())
    )
    assert v.validate_python(MyMapping({"1": 2, 3: "4"})) == {1: 2, 3: 4}
    v = SchemaValidator(
        cs.dict_schema(
            strict=True, keys_schema=cs.int_schema(), values_schema=cs.int_schema()
        )
    )
...
def test_key_error():
    ...
    with pytest.raises(
        ValidationError, match=re.escape("x.[key]\n  Input should be a valid integer")
    ) as exc_info:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {1: "2", 3: "4"},
            Err(
                "Dictionary should have at least 3 items after validation, not 2 [type=too_short,"
            ),
        ),
        ...
        (
            ...
            {"1": 1, "2": 2, "3": 3.0, "4": [1, 2, 3, 4]},
            Err(
                "Dictionary should have at most 3 items after validation, not 4 [type=too_long,"
            ),
        ),
    ],
)
...
def test_dict_complex_key():
    v = SchemaValidator(
        cs.dict_schema(
            keys_schema=cs.complex_schema(strict=True), values_schema=cs.str_schema()
        )
    )
    ...
    v = SchemaValidator(
        cs.dict_schema(keys_schema=cs.complex_schema(), values_schema=cs.str_schema())
    )
    with pytest.raises(
        ValidationError,
...
def test_json_dict_complex_key():
    v = SchemaValidator(
        cs.dict_schema(keys_schema=cs.complex_schema(), values_schema=cs.int_schema())
    )
...
def test_ordered_dict_key_order_preservation():
    # GH 12273
    v = SchemaValidator(
        cs.dict_schema(keys_schema=cs.str_schema(), values_schema=cs.int_schema())
    )
