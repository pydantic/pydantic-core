...
def test_dict_cases(input_value, expected):
    v = SchemaValidator(schema=cs.dict_schema(keys_schema=cs.str_schema(), values_schema=cs.str_schema()))
...
def test_dict_error_key_int():
    v = SchemaValidator(schema=cs.dict_schema(values_schema=cs.int_schema()))
    with pytest.raises(ValidationError, match='Input should be a valid integer') as exc_info:
...
def test_dict_any_value():
    v = SchemaValidator(schema=cs.dict_schema(keys_schema=cs.str_schema()))
    v = SchemaValidator(schema=cs.dict_schema(keys_schema=cs.str_schema()))
...
def test_mapping():
    ...
    v = SchemaValidator(schema=cs.dict_schema(keys_schema=cs.int_schema(), values_schema=cs.int_schema()))
    assert v.validate_python(MyMapping({'1': 2, 3: '4'})) == {1: 2, 3: 4}
    v = SchemaValidator(schema=cs.dict_schema(strict=True, keys_schema=cs.int_schema(), values_schema=cs.int_schema()))
...
def test_dict_length_constraints(kwargs: dict[str, Any], input_value, expected):
    v = SchemaValidator(schema=cs.dict_schema(**kwargs))
    if isinstance(expected, Err):
        with pytest.raises(ValidationError, match=re.escape(expected.message)):
            v.validate_python(input_value)
    else:
...
def test_dict_complex_key():
    v = SchemaValidator(
        schema=cs.dict_schema(keys_schema=cs.complex_schema(strict=True), values_schema=cs.str_schema())
    )
    ...
    v = SchemaValidator(schema=cs.dict_schema(keys_schema=cs.complex_schema(), values_schema=cs.str_schema()))
    with pytest.raises(
...
def test_json_dict_complex_key():
    v = SchemaValidator(schema=cs.dict_schema(keys_schema=cs.complex_schema(), values_schema=cs.int_schema()))
