...
def test_multiple_missing(py_and_json: PyAndJson):
    ...
    with pytest.raises(ValidationError) as exc_info:
        v.validate_test([1])
    assert exc_info.value.errors() == [
        {'type': 'missing', 'loc': [1], 'message': 'Field required', 'input_value': [1]},
        {'type': 'missing', 'loc': [2], 'message': 'Field required', 'input_value': [1]},
        {'type': 'missing', 'loc': [3], 'message': 'Field required', 'input_value': [1]},
    ]
    with pytest.raises(ValidationError) as exc_info:
        v.validate_test([1, 2, 3])
    assert exc_info.value.errors() == [
        {'type': 'missing', 'loc': [3], 'message': 'Field required', 'input_value': [1, 2, 3]}
    ]
...
def test_tuple_fix_error():
    ...
    assert exc_info.value.errors() == [{'type': 'missing', 'loc': [1], 'message': 'Field required', 'input_value': [1]}]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            Err(
                'type=missing',
                errors=[{'type': 'missing', 'loc': [1], 'message': 'Field required', 'input_value': [1]}],
            ),
        ),
    ],
)
...
def test_tuple_fix_extra_any():
    ...
    with pytest.raises(ValidationError) as exc_info:
        v.validate_python([])
    assert exc_info.value.errors() == [{'type': 'missing', 'loc': [0], 'message': 'Field required', 'input_value': []}]
