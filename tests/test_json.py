...
def test_null():
    assert SchemaValidator(schema=core_schema.none_schema()).validate_json('null') is None
...
def test_bytes():
    s = SchemaValidator(schema=core_schema.bytes_schema())
...
def test_float(input_value, expected):
    v = SchemaValidator(schema=core_schema.float_schema())
    if isinstance(expected, Err):
        with pytest.raises(ValidationError, match=re.escape(expected.message)):
            v.validate_json(input_value)
    else:
...
def test_dict():
    v = SchemaValidator(
        schema=core_schema.dict_schema(keys_schema=core_schema.int_schema(), values_schema=core_schema.int_schema())
    )
...
def test_dict_any_value():
    v = SchemaValidator(schema=core_schema.dict_schema(keys_schema=core_schema.str_schema()))
...
def test_inf_nan_allow():
    v = SchemaValidator(schema=core_schema.float_schema(allow_inf_nan=True))
...
def test_json_bytes_base64_round_trip():
    ...
    v = SchemaValidator(schema=core_schema.bytes_schema(), config=CoreConfig(val_json_bytes='base64'))
    ...
    v = SchemaValidator(
        schema=core_schema.dict_schema(keys_schema=core_schema.str_schema(), values_schema=core_schema.bytes_schema()),
...
def test_json_bytes_hex_round_trip():
    ...
    v = SchemaValidator(schema=core_schema.bytes_schema(), config=CoreConfig(val_json_bytes='hex'))
