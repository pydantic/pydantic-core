...
def datetime_schema():
    return SchemaValidator(schema=cs.datetime_schema())
...
def test_pytimedelta_as_timedelta(dt):
    v = SchemaValidator(schema=cs.timedelta_schema(gt=dt))
...
@pytest.fixture(scope='module')
def url_validator():
    return SchemaValidator(schema=cs.url_schema())
...
def multi_host_url_validator():
    return SchemaValidator(schema=cs.multi_host_url_schema())
