...
def test_on_model_class():
    v = SchemaValidator(schema=cs.model_schema(cls=MyModel, config=CoreConfig(str_max_length=5), schema=cs.model_fields_schema(fields={'f': cs.model_field(schema=cs.str_schema())})))
    assert 'max_length:Some(5)' in plain_repr(v)
...
def test_field_priority_model():
    v = SchemaValidator(schema=cs.model_schema(cls=MyModel, config=CoreConfig(str_max_length=10), schema=cs.model_fields_schema(fields={'f': cs.model_field(schema=cs.str_schema(max_length=5))})))
    assert 'max_length:Some(5)' in plain_repr(v)
    assert v.isinstance_python({'f': 'test'}) is True
    assert v.isinstance_python({'f': 'test long'}) is False
...
@pytest.mark.parametrize(
    ...
    [
        (CoreConfig(), cs.float_schema(), {'x': 'nan'}, IsInstance(MyModel) & HasAttributes(x=FunctionCheck(math.isnan))),
        (
...
def test_allow_inf_nan(config: CoreConfig, float_field_schema, input_value, expected):
    v = SchemaValidator(schema=cs.model_schema(cls=MyModel, schema=cs.model_fields_schema(fields={'x': cs.model_field(schema=float_field_schema)}), config=config))
...
def test_hide_input_in_errors(config, input_str):
    v = SchemaValidator(schema=cs.model_schema(cls=MyModel, schema=cs.model_fields_schema(fields={'f': cs.model_field(schema=cs.str_schema())})), config=config)
